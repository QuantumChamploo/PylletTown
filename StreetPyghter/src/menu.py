...
def getCharList():
    folder = '../StreetPyghter/src/../res/Char'
...
def getBckgrndList():
    folder = '../StreetPyghter/src/../res/Background'
    list = os.listdir(folder)
...
class Value:
    def __init__(self, string, value = 2, maxValue = 5, position = Point(0,0)):
        ...
        self.dot = pygame.image.load('../StreetPyghter/src/../res/dot.png').convert_alpha()
        
    def convert(self):
        filled = pygame.image.load('../StreetPyghter/src/../res/dotempty.png').convert_alpha()
...
class Menu:
    def __init__(self, position, screen, background = '../StreetPyghter/src/MenuScreen.png'):
        self.sprites = config.Alphabet().sprites
#        self.sprites = SpriteSheetLoader('../res/Ascii.png', 16, 16).getSpriteList()
        self.cursor = pygame.image.load('../StreetPyghter/src/../res/cursor.png').convert_alpha()
        self.screen = screen
        self.options = []
        self.position = position
        self.choice = 1
    ...
    def mainloop(self):
        background = pygame.image.load('../StreetPyghter/src/../res/Background/'+self.background).convert()
        while True:
...
class KeyMenu(Menu):
    def __init__(self, string,  screen, value):
        ...
        self.waiting = pygame.image.load('../StreetPyghter/src/../res/waiting.png')
...
class CharMenu():
    ...
    def update(self):
        self.portrait = pygame.image.load('../StreetPyghter/src/../res/Char/'+self.list[self.mugs_start+self.cursor_pos]+'/portrait.png').convert_alpha()
        
        length = len(self.list)
        self.mug1 = pygame.image.load('../StreetPyghter/src/../res/Char/'+self.list[self.mugs_start]+'/mug.png').convert_alpha()
        if self.mugs_start+1 < length:
            self.mug2 = pygame.image.load('../StreetPyghter/src/../res/Char/'+self.list[self.mugs_start+1]+'/mug.png').convert_alpha()
        else: self.mug2 = None
        if self.mugs_start+2 < length:
            self.mug3 = pygame.image.load('../StreetPyghter/src/../res/Char/'+self.list[self.mugs_start+2]+'/mug.png').convert_alpha()
        else: self.mug3 = None
        if self.mugs_start+3 < length:
            self.mug4 = pygame.image.load('../StreetPyghter/src/../res/Char/'+self.list[self.mugs_start+3]+'/mug.png').convert_alpha()
        else: self.mug4 = None
...
class CharSelectMenu(Menu):
    def __init__(self, screen, KeysP1, KeysP2, position = Point(0,0)):
        ...
        self.keysP1 = KeysP1
        self.keysP2 = KeysP2
        self.cursor_sprites = game.GameObject('../StreetPyghter/src/../res/charcursor.png', 36, 44, Point(0,0)).sprite_list
        self.charmenuP1 = CharMenu(charlist, self.cursor_sprites, True)
        self.charmenuP2 = CharMenu(charlist, self.cursor_sprites, False)
    
    def mainloop(self):
        background = pygame.image.load('../StreetPyghter/src/../res/Background/SelectScreen.png').convert()
...
class Credits():
    ...
    def mainloop(self):
        background = pygame.image.load('../StreetPyghter/src/../res/Background/OptionScreen.png').convert()
        clock = pygame.time.Clock()
        self.tick = 0
...
class OptionMenu(Menu):
    def __init__(self, screen, position = Point(0,0)):
        self.config = config.OptionConfig('../StreetPyghter/src/../res/config.txt')
...
class MainMenu(Menu):
    ...
    def call_fight(self, player1, player2):
        ...
        background = Round.Background('../StreetPyghter/src/../res/Background/'+list[rand])
